...
// Simple MD5 implementation (browser-compatible)
function md5(str: string): string {
  ...
  const string8 = unescape(encodeURIComponent(str))
  const length8 = string8.length
  const w: number[] = []
  const HEX_CHARS = '0123456789abcdef'
  const result: number[] = []
  let i, h0 = 1732584193, h1 = -271733879, h2 = -1732584194, h3 = 271733878
  const words = new Array(Math.ceil((length8+9)/64)*16).fill(0)
  for (i=0;i<length8;i++) words[i>>2] |= string8.charCodeAt(i) << (i%4*8)
  words[length8>>2] |= 0x80 << (length8%4*8)
  words[words.length-2] = length8*8
  for (i=0;i<words.length;i+=16) {
    ...
    h0=safeAdd(a,h0); h1=safeAdd(b,h1); h2=safeAdd(c,h2); h3=safeAdd(d,h3)
  }
  const hex = (n: number) => { let s=''; for(let i=0;i<4;i++) s+=HEX_CHARS[(n>>(i*8+4))&0xF]+HEX_CHARS[(n>>(i*8))&0xF]; return s }
  return hex(h0)+hex(h1)+hex(h2)+hex(h3)
}

export default function Md5Generator({ accent }: { accent?: string }) {
    ...
    <div className="space-y-4">
      <div className="tool-box">
        ...
        <textarea value={input} onChange={e => setInput(e.target.value)}
          ...
          placeholder="Enter any text to generate its MD5 hash…" />
        <button onClick={() => setInput('')} className="btn btn-ghost btn-sm mt-3">Clear</button>
      </div>
      {hash && (
        <div className="space-y-3">
          {[
            { label: 'MD5 (Hex)', value: hash, color: '#D85A30' },
            { label: 'MD5 (Uppercase)', value: hash.toUpperCase(), color: '#7F77DD' },
          ].map(item => (
            <div key={item.label} className="tool-box">
              <label className="form-label" style={{ color: item.color }}>{item.label}</label>
              <div className="tool-output text-sm font-mono break-all">{item.value}</div>
          ...
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl px-4 py-3 text-xs text-amber-800 dark:text-amber-300">
            ⚠️ MD5 is not cryptographically secure. Use SHA-256 for security-critical applications.
          </div>
        </div>
      )}
    </div>
